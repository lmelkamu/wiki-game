...
let find_friend_group network ~person : Person.t list =
  let graph =
    Set.fold network ~init:Person.Map.empty ~f:(fun init (p1, p2) ->
      Map.update init p1 ~f:(fun neighbors ->
        match neighbors with Some list -> list @ [ p2 ] | None -> [ p2 ]))
  in
  let friends = Person.Hash_set.create () in
  ...
  let rec traverse () =
    ...
    | None -> ()
    | Some current_node ->
      if not (Hash_set.mem friends current_node)
      then (
        Hash_set.add friends current_node;
